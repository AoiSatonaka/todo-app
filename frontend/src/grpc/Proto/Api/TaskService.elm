{- !!! DO NOT EDIT THIS FILE MANUALLY !!! -}

module Proto.Api.TaskService exposing (add, delete, get, list, update)

{-| 
This file was automatically generated by
- [`protoc-gen-elm`](https://www.npmjs.com/package/protoc-gen-elm) 3.2.0
- `protoc` 4.24.4
- the following specification files: `task.proto`

To run it, add a dependency via `elm install` on [`elm-protocol-buffers`](https://package.elm-lang.org/packages/eriktim/elm-protocol-buffers/1.2.0) version latest or higher.


@docs add, delete, get, list, update
-}

import Grpc.Internal
import Proto.Api


{-| A template for a gRPC call to the method 'Delete' sending a `TaskDeleteRequest` to get back a `TaskListResponse`.


-}
delete : Grpc.Internal.Rpc Proto.Api.TaskDeleteRequest Proto.Api.TaskListResponse
delete =
    Grpc.Internal.Rpc
        { service = "TaskService"
        , package = "api"
        , rpcName = "Delete"
        , encoder = Proto.Api.encodeTaskDeleteRequest
        , decoder = Proto.Api.decodeTaskListResponse
        }


{-| A template for a gRPC call to the method 'Update' sending a `Task` to get back a `TaskListResponse`.


-}
update : Grpc.Internal.Rpc Proto.Api.Task Proto.Api.TaskListResponse
update =
    Grpc.Internal.Rpc
        { service = "TaskService"
        , package = "api"
        , rpcName = "Update"
        , encoder = Proto.Api.encodeTask
        , decoder = Proto.Api.decodeTaskListResponse
        }


{-| A template for a gRPC call to the method 'Add' sending a `TaskAddRequest` to get back a `TaskListResponse`.


-}
add : Grpc.Internal.Rpc Proto.Api.TaskAddRequest Proto.Api.TaskListResponse
add =
    Grpc.Internal.Rpc
        { service = "TaskService"
        , package = "api"
        , rpcName = "Add"
        , encoder = Proto.Api.encodeTaskAddRequest
        , decoder = Proto.Api.decodeTaskListResponse
        }


{-| A template for a gRPC call to the method 'Get' sending a `TaskGetRequest` to get back a `Task`.


-}
get : Grpc.Internal.Rpc Proto.Api.TaskGetRequest Proto.Api.Task
get =
    Grpc.Internal.Rpc
        { service = "TaskService"
        , package = "api"
        , rpcName = "Get"
        , encoder = Proto.Api.encodeTaskGetRequest
        , decoder = Proto.Api.decodeTask
        }


{-| A template for a gRPC call to the method 'List' sending a `TaskListRequest` to get back a `TaskListResponse`.


-}
list : Grpc.Internal.Rpc Proto.Api.TaskListRequest Proto.Api.TaskListResponse
list =
    Grpc.Internal.Rpc
        { service = "TaskService"
        , package = "api"
        , rpcName = "List"
        , encoder = Proto.Api.encodeTaskListRequest
        , decoder = Proto.Api.decodeTaskListResponse
        }
